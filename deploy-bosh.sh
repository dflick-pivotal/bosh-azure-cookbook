...
if [ -d "recipes/$recipe" ]; then
  ...
  if [ "$loadBalancer" -gt 0 ]; then
    ...
    while [ "$i" -lt "$lbRuleCount" ]; do
      ...
      az network lb rule create -g $vmResGroup --lb-name $lbName -n kubo-api --protocol TCP --frontend-port 8443 --backend-port 8443 --frontend-ip-name LoadBalancerFrontEnd --backend-pool-name kubo-lbbepool
      i=$(($i + 1))
    done
